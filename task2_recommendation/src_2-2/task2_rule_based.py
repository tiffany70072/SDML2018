...
def predict_last_day_2times():
	...
	for i in range(user_num):
		...
		for j in range(len(table_list[i])-1, -1, -1):
			for k in range(len(table_list[i][j])):
				if table_list[i][j][k] not in answer_one: 
					answer_one.append(table_list[i][j][k])
...
def prepare_ranking_foodid(num_class=300):
	...
	for i in range(user_num): # user_num
		...
		rank_sort = -(np.sort(rank_array)[:num_class]) 
		for j in range(num_class):
			if rank_sort[j] > 0: 
				foodid_rank_each_user[i][j] = rank_argsort[j]
			else: 
				break
	...
	for i in range(user_num):
		for j in range(num_class):
			if foodid_rank_each_user[i][j] == -1: 
				break
