...
'''import sys
from keras.utils import inspect
...
class RNN(Layer):
    ...
    def call(self,
        ...
        kwargs = {}
        ###### debug here, cannot import has_arg
        print(self.cell.call)
...
class GRUCell(Layer):
    ...
    def __init__(self, units,
                 ...
                 nan_initializer='glorot_uniform',
                 kernel_regularizer=None,
                 recurrent_regularizer=None,
                 bias_regularizer=None,
                 nan_regularizer=None,
        ...
        self.nan_initializer = initializers.get(nan_initializer)

        self.kernel_regularizer = regularizers.get(kernel_regularizer)
        self.recurrent_regularizer = regularizers.get(recurrent_regularizer)
        self.bias_regularizer = regularizers.get(bias_regularizer)
        self.nan_regularizer = regularizers.get(nan_regularizer)

        self.kernel_constraint = constraints.get(kernel_constraint)
        self.recurrent_constraint = constraints.get(recurrent_constraint)
        self.bias_constraint = constraints.get(bias_constraint)
        self.nan_constraint = constraints.get(nan_constraint)
    ...
    def build(self, input_shape):
        ...
        input_dim = input_shape[-1]
        self.kernel = self.add_weight(shape=(int(input_dim/2.0), self.units * 3), # W_kernel
                                      name='kernel',
                                      initializer=self.kernel_initializer,
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        self.recurrent_kernel = self.add_weight( # U_kernel
        ...
        self.nan_kernel = self.add_weight(shape=(int(input_dim/2.0), self.units * 3), # W_kernel
                                      name='nan_kernel',
                                      initializer=self.nan_initializer,
                                      regularizer=self.nan_regularizer,
                                      constraint=self.nan_constraint)
        ...
        self.nan_kernel_z = self.nan_kernel[:, :self.units]

        # reset gate
        self.kernel_r = self.kernel[:, self.units: self.units * 2]
        self.recurrent_kernel_r = self.recurrent_kernel[:, self.units:self.units * 2]
        self.nan_kernel_r = self.nan_kernel[:, self.units: self.units * 2]

        # new gate
        self.kernel_h = self.kernel[:, self.units * 2:]
        self.recurrent_kernel_h = self.recurrent_kernel[:, self.units * 2:]
        self.nan_kernel_h = self.nan_kernel[:, self.units * 2:]
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            if 0. < self.dropout < 1.:
                ...
                inputs_z = inputs[:, 0:int(self.input_dim/2.0)]
                inputs_r = inputs[:, 0:int(self.input_dim/2.0)]
                inputs_h = inputs[:, 0:int(self.input_dim/2.0)]
                m = inputs[:, int(self.input_dim/2.0):self.input_dim] # is nan or not
                #print('debug, inputs =', inputs_z.shape)
                #print('debug inputs_z = tf.concat([inputs[:, 1:2], inputs[:, 1:2]], 1)')

            x_z = K.dot(inputs_z, self.kernel_z) + K.dot(m, self.nan_kernel_z) # Vz * m
            x_r = K.dot(inputs_r, self.kernel_r) + K.dot(m, self.nan_kernel_r)
            x_h = K.dot(inputs_h, self.kernel_h) + K.dot(m, self.nan_kernel_h)
...
class GRU(RNN):
    """G   There are two variants. The default one is based on 1406.1078v3 and
