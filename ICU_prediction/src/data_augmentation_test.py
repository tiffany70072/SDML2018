...
def plot_pca():
	...
	for j in range(0, x_train.shape[0], 100):
		if j < icu_num: color = 'r'#idx = 0
		elif j < x_train.shape[0]: color = 'g' #idx = 1
		elif j < x_train.shape[0] + x_adv.shape[0]: color = 'b' #idx = 2
		#elif j < x_train.shape[0] + x_valid.shape[0]: idx = 3
		#elif j < x_train.shape[0] + x_valid.shape[0] + x_adv.shape[0]/2: idx = 4
		#else: idx = 5
		...
		plt.show() 
		#plt.savefig('examples/plt/length_cov_pca_10000_200_-2')
		#plt.savefig('examples/plt/length_cov_pca_10000_200_dim50_ave_notran')
		plt.savefig('plt/0106_only_train')

		if j % 10000 == 0: print(j)

def train():
	model_type = 'mlp+rnn' # one of 'mlp', 'rnn', , 'multi', 'cnn'
	window_size = 8
	basicInf_train, history_train, y_train = read_saved_data(is_train = 1, window_size = window_size)
	x_train = preprocess_training_data(basicInf_train, history_train, model_type, window_size)
	basicInf_valid, history_valid, y_valid = read_saved_data(is_train = 0, window_size = window_size)
	...
	for i in range(1, 11):
		if i == 1: x_adv = np.load('adv_fgsm/adv_fgsm_' + str(i) + '.npy')
		else: x_adv = np.r_[x_adv, np.load('adv_fgsm/adv_fgsm_' + str(i) + '.npy')]
