"""Construct model, such as d-GRU, and other utility function."""


import numpy as np

# TODO: from dGRU import GRU as dGRU
...
def naive_rnn_model(max_length=64, features_length=5, cell_type='gru', units=256):
	...
	if cell_type == 'lstm': 
		x = LSTM(units, input_shape=(max_length, features_length), return_sequences=0)(inputs)
	elif cell_type == 'gru': 
		x = GRU(units, return_sequences=0)(inputs)
	elif cell_type == 'rnn': 
		x = SimpleRNN(units, return_sequences=0)(inputs)
...
def mlp_rnn_model(input_mlp_shape=4, max_length=64, features_length=5, units=256): 
	# Concat[1 layer mlp, 1 layer rnn] to 1 mlp.
...
def get_model(model_type, window_size, features_length=5):
	if model_type == 'mlp': 
		model = naive_mlp_model(input_shape=window_size*5+4)
	elif model_type == 'rnn': 
		model = naive_rnn_model(max_length=window_size, features_length=features_length)
	elif model_type == 'mlp+rnn': 
		model = mlp_rnn_model(max_length=window_size, features_length=features_length)
	elif model_type == 'multi': 
		model = multi_mlp_rnn_model(max_length=window_size, input_mlp_shape=window_size*5+4, features_length=features_length)
	elif model_type == 'cnn': 
		model = mlp_cnn_model(input_mlp_shape=window_size*5+4, max_length=window_size)
	else:
		print('no this model')
		exit()
	model.summary()
